...
func newImageOutput(i unsafe.Pointer) *ImageOutput {
	holder := &i
	in := &ImageOutput{ptr: holder}
	runtime.AddCleanup(in, cleanupImageOutput, holder)
	return in
}

func cleanupImageOutput(ptr *unsafe.Pointer) {
	p := atomic.SwapPointer(ptr, nil)
	if p != nil {
		C.deleteImageOutput(p)
	}
}

func (i *ImageOutput) p() unsafe.Pointer {
	return atomic.LoadPointer(i.ptr)
}

// OpenImageOutput creates an ImageOutput that will write to a file, with the format.
...
func (i *ImageOutput) LastError() error {
	c_str := C.ImageOutput_geterror(i.p())
	runtime.KeepAlive(i)
	if c_str == nil {
		return nil
	}
...
func (i *ImageOutput) Supports(feature string) bool {
	...
	defer C.free(unsafe.Pointer(c_str))
	ret := bool(C.ImageOutput_supports(i.p(), c_str))
	runtime.KeepAlive(i)
	return ret
}
...
func (i *ImageOutput) FormatName() string {
	ret := C.GoString(C.ImageOutput_format_name(i.p()))
	runtime.KeepAlive(i)
	return ret
}
