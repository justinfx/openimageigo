...
// ImageCache define an api to an abstract class that manages image files, caches of open.
...
// CreateImageCache creates an ImageCache. *This should be freed by calling ImageCache.Destroy()*.
//
...
// Clear closes everything, free resources, start from scratch.
func (i *ImageCache) Clear() {
	C.ImageCache_clear(i.ptr)
	runtime.KeepAlive(i)
}

// GetStats returns the statistics output as a huge string..
...
// ResetStats resets most statistics to be as they were with a fresh ImageCache.
...
// InvalidateAll invalidate all loaded tiles and open file handles. this is safe to do even if other.
