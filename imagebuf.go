...
// IBStorage description of where the pixels live for this imagebuf.
type IBStorage int

const (
	// FileFormatAuto derive the file format from the file path name (empty string).
	FileFormatAuto = ""
...
func newImageBuf(i unsafe.Pointer) *ImageBuf {
	holder := &i
	spec := &ImageBuf{ptr: holder}
	runtime.AddCleanup(spec, cleanupImageBuf, holder)
	return spec
}

func cleanupImageBuf(ptr *unsafe.Pointer) {
...
func (i *ImageBuf) p() unsafe.Pointer {
	return atomic.LoadPointer(i.ptr)
}

// LastError returns the last error generated by API calls..
// An nil error will be returned if no error has occured.
func (i *ImageBuf) LastError() error {
	c_str := C.ImageBuf_geterror(i.p())
	if c_str == nil {
		return nil
	}
	runtime.KeepAlive(i)
...
// NewImageBuf construct an empty/uninitialized imagebuf. this is relatively useless until you call reset().
func NewImageBuf() *ImageBuf {
	buf := C.ImageBuf_New()
	return newImageBuf(buf)
}

// NewImageBufPath construct an imagebuf to read the named image - but don't actually read it yet!
...
// NewImageBufPathCache construct an imagebuf to read the named image - but don't actually read it yet!
// The image will actually be read when other methods need to access the spec and/or pixels,
// or when an explicit call to init_spec() or read() is made, whichever comes first.
//
...
// NewImageBufSpec construct an imagebuf given a proposed spec describing the image size and type,.
...
func NewImageBufSpec(spec *ImageSpec) (*ImageBuf, error) {
	buf := newImageBuf(C.ImageBuf_New_Spec(spec.p()))
...
// Initialized returns true if this ImageBuf object initialized?
func (i *ImageBuf) Initialized() bool {
	if i.ptr == nil {
		return false
	}
	ret := bool(C.ImageBuf_initialized(i.p()))
	runtime.KeepAlive(i)
	return ret
}
...
func (i *ImageBuf) Destroy() {
	cleanupImageBuf(i.ptr)
	runtime.KeepAlive(i)
}

// Clear restore the imagebuf to an uninitialized state.
func (i *ImageBuf) Clear() {
	if i.ptr == nil {
		return
	}
	C.ImageBuf_clear(i.p())
	runtime.KeepAlive(i)
}

func (i *ImageBuf) Storage() IBStorage {
	ret := IBStorage(C.ImageBuf_storage(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// InitSpec initializes this ImageBuf with the named image file, and read its header to.
...
func (i *ImageBuf) InitSpec(filename string, subimage, miplevel int) error {
	...
	ok := bool(C.ImageBuf_init_spec(i.p(), c_str, C.int(subimage), C.int(miplevel)))
	if !ok {
		return i.LastError()
	}
	runtime.KeepAlive(i)
...
func (i *ImageBuf) Reset(filename string, cache *ImageCache) error {
	...
	C.ImageBuf_reset_name_cache(i.p(), c_str, cachePtr)
	err := i.LastError()
	runtime.KeepAlive(i)
	return err
}
...
func (i *ImageBuf) ResetSubImage(filename string, subimage, miplevel int, cache *ImageCache, config *ImageSpec) error {
	...
	C.ImageBuf_reset_subimage(i.p(), c_str, C.int(subimage), C.int(miplevel), cachePtr, specPtr)
	err := i.LastError()
	runtime.KeepAlive(i)
	return err
}
...
func (i *ImageBuf) ResetSpec(spec *ImageSpec) error {
	if spec == nil || spec.p() == nil {
		return errors.New("nil ImageSpec")
	}

	var specPtr unsafe.Pointer = spec.p()

	C.ImageBuf_reset_spec(i.p(), specPtr)
	err := i.LastError()
	runtime.KeepAlive(i)
	return err
}
...
func (i *ImageBuf) ResetNameSpec(filename string, spec *ImageSpec) error {
	...
	C.ImageBuf_reset_name_spec(i.p(), c_str, specPtr)
	err := i.LastError()
	runtime.KeepAlive(i)
	return err
}
...
// ReadSubImage reads the file from disk. Generally will skip the read if we've already got a current.
// version of the image in memory, unless force==true.
// This uses ImageInput underneath, so will read any file format for which an appropriate
// imageio plugin can be found.
...
// ReadCallback reads the file from disk. Generally will skip the read if we've already got a current.
// version of the image in memory, unless force==true.
// This uses ImageInput underneath, so will read any file format for which an appropriate
// imageio plugin can be found.
//
...
// ReadSubImageCallback reads the file from disk. Generally will skip the read if we've already got a current.
// version of the image in memory, unless force==true.
// This uses ImageInput underneath, so will read any file format for which an appropriate
// imageio plugin can be found.
//
...
// ReadFormatCallback reads the file from disk. Generally will skip the read if we've already got a current.
// version of the image in memory, unless force==true.
// This uses ImageInput underneath, so will read any file format for which an appropriate
// imageio plugin can be found.
//
...
func (i *ImageBuf) ReadFormatCallback(force bool, convert TypeDesc, progress *ProgressCallback) error {
	...
	ok := C.ImageBuf_read(i.p(), 0, 0, C.bool(force), C.TypeDesc(convert), cbk)
	if !bool(ok) {
		return i.LastError()
	}
	runtime.KeepAlive(i)
...
// ReadSubImageFormatCallback reads the file from disk. Generally will skip the read if we've already got a current.
// version of the image in memory, unless force==true.
// This uses ImageInput underneath, so will read any file format for which an appropriate
// imageio plugin can be found.
//
...
func (i *ImageBuf) ReadSubImageFormatCallback(subimage, miplevel int, force bool, convert TypeDesc, progress *ProgressCallback) error {
	...
	ok := C.ImageBuf_read(i.p(), C.int(subimage), C.int(miplevel), C.bool(force), C.TypeDesc(convert), cbk)
	if !bool(ok) {
		return i.LastError()
	}
	runtime.KeepAlive(i)
...
// WriteFile writes the image to the named file and file format.
...
// WriteFileProgress writes the image to the named file and file format.
// (fileformat=="" means to infer the type from the filename extension).
//
// This call optionally supports passing a callback pointer to both track the progress,
// and to optionally abort the processing. The callback function will receive
...
func (i *ImageBuf) WriteFileProgress(filepath, fileformat string, progress *ProgressCallback) error {
	...
	ok := C.ImageBuf_write_file(i.p(), c_path, c_fmt, cbk)
	if !bool(ok) {
		return i.LastError()
	}
	runtime.KeepAlive(i)
...
// WriteImageOutput writes the image to the open ImageOutput 'out'. Return true if all went ok, false if there were errors writing.
...
// WriteImageOutputProgress writes the image to the open ImageOutput 'out'. Return true if all went ok, false if there were errors writing.
// It does NOT close the file when it's done (and so may be called in a loop to write a multi-image file).
//
// This call optionally supports passing a callback pointer to both track the progress,
// and to optionally abort the processing. The callback function will receive
...
func (i *ImageBuf) WriteImageOutputProgress(output *ImageOutput, progress *ProgressCallback) error {
	...
	ok := C.ImageBuf_write_output(i.p(), output.p(), cbk)
	if !bool(ok) {
		return i.LastError()
	}
	runtime.KeepAlive(i)
...
// SetWriteFormat inform the imagebuf what data format you'd like for any subsequent write().
func (i *ImageBuf) SetWriteFormat(format TypeDesc) {
	C.ImageBuf_set_write_format(i.p(), C.TypeDesc(format))
	runtime.KeepAlive(i)
}

// SetWriteTiles inform the imagebuf what tile size (or no tiling, for 0) for any subsequent write*().
func (i *ImageBuf) SetWriteTiles(width, height, depth int) {
	C.ImageBuf_set_write_tiles(i.p(), C.int(width), C.int(height), C.int(depth))
	runtime.KeepAlive(i)
}

// CopyMetadata copy all the metadata from src to this (except for pixel data resolution,.
// channel information, and data format).
func (i *ImageBuf) CopyMetadata(src *ImageBuf) error {
	C.ImageBuf_copy_metadata(i.p(), src.p())
	runtime.KeepAlive(i)
	runtime.KeepAlive(src)
	return i.LastError()
}

// CopyPixels copy the pixel data from src to this, automatically converting to the existing data.
...
func (i *ImageBuf) CopyPixels(src *ImageBuf) error {
	ok := bool(C.ImageBuf_copy_pixels(i.p(), src.p()))
	runtime.KeepAlive(i)
	runtime.KeepAlive(src)
	if !ok {
		return i.LastError()
...
// Copy try to copy the pixels and metadata from src to this, returning true upon success.
...
func (i *ImageBuf) Copy(src *ImageBuf) error {
	ok := bool(C.ImageBuf_copy(i.p(), src.p()))
	runtime.KeepAlive(i)
	runtime.KeepAlive(src)
	if !ok {
		return i.LastError()
...
func (i *ImageBuf) Swap(other *ImageBuf) error {
	C.ImageBuf_swap(i.p(), other.p())
	runtime.KeepAlive(other)
	runtime.KeepAlive(i)
	return i.LastError()
}

// Spec returns a reference to the image spec that describes the buffer..
...
// NativeSpec returns a reference to the "native" image spec (that describes the file, which may be slightly.
...
// SpecMod returns a writable reference to the image spec that describes the buffer..
...
func (i *ImageBuf) SpecMod() *ImageSpec {
	ret := newImageSpecBorrowed(C.ImageBuf_specmod(i.p()))
	runtime.KeepAlive(i)
	return ret
}
...
func (i *ImageBuf) Name() string {
	ret := C.GoString(C.ImageBuf_name(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// FileFormatName returns the name of the image file format of the disk file we read into this image..
// Returns an empty string if this image was not the result of a Read().
func (i *ImageBuf) FileFormatName() string {
	ret := C.GoString(C.ImageBuf_file_format_name(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// SubImage returns the index of the subimage are we currently viewing.
func (i *ImageBuf) SubImage() int {
	ret := int(C.ImageBuf_subimage(i.p()))
	runtime.KeepAlive(i)
	return ret
}
...
func (i *ImageBuf) NumSubImages() int {
	ret := int(C.ImageBuf_nsubimages(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// MipLevel returns the index of the miplevel are we currently viewing.
func (i *ImageBuf) MipLevel() int {
	ret := int(C.ImageBuf_miplevel(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// NumMipLevels returns the number of miplevels of the current subimage..
func (i *ImageBuf) NumMipLevels() int {
	ret := int(C.ImageBuf_nmiplevels(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// NumChannels returns the number of color channels in the image..
func (i *ImageBuf) NumChannels() int {
	ret := int(C.ImageBuf_nchannels(i.p()))
	runtime.KeepAlive(i)
	return ret
}
...
// Orientation by default, image pixels are ordered from the top of the display to the bottom, and within.
...
func (i *ImageBuf) Orientation() int {
	ret := int(C.ImageBuf_orientation(i.p()))
	runtime.KeepAlive(i)
	return ret
}

func (i *ImageBuf) OrientedWidth() int {
	ret := int(C.ImageBuf_oriented_width(i.p()))
	runtime.KeepAlive(i)
	return ret
}

func (i *ImageBuf) OrientedHeight() int {
	ret := int(C.ImageBuf_oriented_height(i.p()))
	runtime.KeepAlive(i)
	return ret
}

func (i *ImageBuf) OrientedX() int {
	ret := int(C.ImageBuf_oriented_x(i.p()))
	runtime.KeepAlive(i)
	return ret
}

func (i *ImageBuf) OrientedY() int {
	ret := int(C.ImageBuf_oriented_y(i.p()))
	runtime.KeepAlive(i)
	return ret
}

func (i *ImageBuf) OrientedFullWidth() int {
	ret := int(C.ImageBuf_oriented_full_width(i.p()))
	runtime.KeepAlive(i)
	return ret
}

func (i *ImageBuf) OrientedFullHeight() int {
	ret := int(C.ImageBuf_oriented_full_height(i.p()))
	runtime.KeepAlive(i)
	return ret
}

func (i *ImageBuf) OrientedFullX() int {
	ret := int(C.ImageBuf_oriented_full_x(i.p()))
	runtime.KeepAlive(i)
	return ret
}

func (i *ImageBuf) OrientedFullY() int {
	ret := int(C.ImageBuf_oriented_full_y(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// XBegin returns the beginning (minimum) x coordinate of the defined image..
func (i *ImageBuf) XBegin() int {
	ret := int(C.ImageBuf_xbegin(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// XEnd returns the end (one past maximum) x coordinate of the defined image..
func (i *ImageBuf) XEnd() int {
	ret := int(C.ImageBuf_xend(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// YBegin returns the beginning (minimum) y coordinate of the defined image.
func (i *ImageBuf) YBegin() int {
	ret := int(C.ImageBuf_ybegin(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// YEnd returns the end (one past maximum) y coordinate of the defined image..
func (i *ImageBuf) YEnd() int {
	ret := int(C.ImageBuf_yend(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// ZBegin returns the beginning (minimum) z coordinate of the defined image..
func (i *ImageBuf) ZBegin() int {
	ret := int(C.ImageBuf_zbegin(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// ZEnd returns the end (one past maximum) z coordinate of the defined image..
func (i *ImageBuf) ZEnd() int {
	ret := int(C.ImageBuf_zend(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// XMin returns the end (one past maximum) z coordinate of the defined image..
func (i *ImageBuf) XMin() int {
	ret := int(C.ImageBuf_xmin(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// XMax returns the maximum x coordinate of the defined image..
func (i *ImageBuf) XMax() int {
	ret := int(C.ImageBuf_xmax(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// YMin returns the minimum y coordinate of the defined image..
func (i *ImageBuf) YMin() int {
	ret := int(C.ImageBuf_ymin(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// YMax returns the maximum y coordinate of the defined image..
func (i *ImageBuf) YMax() int {
	ret := int(C.ImageBuf_ymax(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// ZMin returns the minimum z coordinate of the defined image..
func (i *ImageBuf) ZMin() int {
	ret := int(C.ImageBuf_zmin(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// ZMax returns the maximum z coordinate of the defined image..
func (i *ImageBuf) ZMax() int {
	ret := int(C.ImageBuf_zmax(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// SetFull sets the "full" (a.k.a. display) window to [xbegin,xend) x [ybegin,yend) x [zbegin,zend)..
...
// ROI returns pixel data window for this ImageBuf as a ROI.
func (i *ImageBuf) ROI() *ROI {
	ret := newROI(C.ImageBuf_roi(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// ROIFull returns full/display window for this ImageBuf as a ROI.
func (i *ImageBuf) ROIFull() *ROI {
	ret := newROI(C.ImageBuf_roi_full(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// SetROIFull sets full/display window for this ImageBuf to a ROI. Does NOT change the channels of the spec,.
// regardless of newroi.
func (i *ImageBuf) SetROIFull(roi *ROI) error {
	C.ImageBuf_set_roi_full(i.p(), roi.p())
	runtime.KeepAlive(i)
	runtime.KeepAlive(roi)
	return i.LastError()
}
...
func (i *ImageBuf) PixelsValid() bool {
	ret := bool(C.ImageBuf_pixels_valid(i.p()))
	runtime.KeepAlive(i)
	return ret
}

func (i *ImageBuf) PixelType() TypeDesc {
	ret := TypeDesc(C.ImageBuf_pixeltype(i.p()))
	runtime.KeepAlive(i)
	return ret
}

func (i *ImageBuf) CachedPixels() bool {
	ret := bool(C.ImageBuf_cachedpixels(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// ImageCache returns the ImageCache in which backs this ImageBuf.
...
// Deep checks if this ImageBuf store deep data?
func (i *ImageBuf) Deep() bool {
	ret := bool(C.ImageBuf_deep(i.p()))
	runtime.KeepAlive(i)
	return ret
}
