...
func newImageInput(i unsafe.Pointer) *ImageInput {
	holder := &i
	in := &ImageInput{ptr: holder}
	runtime.AddCleanup(in, cleanupImageInput, holder)
	return in
}

func cleanupImageInput(ptr *unsafe.Pointer) {
	p := atomic.SwapPointer(ptr, nil)
	if p != nil {
		C.ImageInput_close(p)
		C.deleteImageInput(p)
	}
}

func (i *ImageInput) p() unsafe.Pointer {
	return atomic.LoadPointer(i.ptr)
}

// OpenImageInput creates an ImageInput subclass instance that is able to read the given file and open it,.
...
func (i *ImageInput) LastError() error {
	c_str := C.ImageInput_geterror(i.p())
	runtime.KeepAlive(i)
	if c_str == nil {
		return nil
	}
...
func (i *ImageInput) Open(filename string) error {
	...
	// Clean up old pointer before assigning new one
	oldPtr := atomic.SwapPointer(i.ptr, nil)
	if oldPtr != nil {
		C.deleteImageInput(oldPtr)
	}
...
func (i *ImageInput) Close() error {
	if !bool(C.ImageInput_close(i.p())) {
		return i.LastError()
	}
	return nil
}

// FormatName returns the name of the format implemented by this image..
func (i *ImageInput) FormatName() string {
	ret := C.GoString(C.ImageInput_format_name(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// ValidFile returns true if the named file is file of the type for this ImageInput.
...
func (i *ImageInput) ValidFile(filename string) bool {
	c_str := C.CString(filename)
	defer C.free(unsafe.Pointer(c_str))
	ret := bool(C.ImageInput_valid_file(i.p(), c_str))
	runtime.KeepAlive(i)
	return ret
}

// Supports given the name of a 'feature', return whether this imageoutput.
...
func (i *ImageInput) Supports(feature string) bool {
	...
	defer C.free(unsafe.Pointer(c_str))
	ret := bool(C.ImageInput_supports(i.p(), c_str))
	runtime.KeepAlive(i)
	return ret
}

// Spec returns a reference to the image format specification of the current subimage/MIPlevel..
...
func (i *ImageInput) Spec() *ImageSpec {
	ptr := C.ImageInput_spec(i.p())
	runtime.KeepAlive(i)
	return newImageSpecBorrowed(ptr)
}
...
func (i *ImageInput) CurrentSubimage() int {
	ret := int(C.ImageInput_current_subimage(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// SeekSubimage seek to the given subimage within the open image file.
...
func (i *ImageInput) SeekSubimage(index int, newSpec *ImageSpec) bool {
	...
	if newSpec == nil || newSpec.p() == nil {
		newSpec = NewImageSpec(TypeUnknown)
	}

	ok := C.ImageInput_seek_subimage(i.p(), C.int(index), newSpec.p())
	runtime.KeepAlive(i)
	runtime.KeepAlive(newSpec)
	return bool(ok)
}

// CurrentMipLevel returns the index of the MIPmap image that is currently being read.
// The highest-res MIP level (or the only level, if there is just
// one) is number 0.
func (i *ImageInput) CurrentMipLevel() int {
	ret := int(C.ImageInput_current_miplevel(i.p()))
	runtime.KeepAlive(i)
	return ret
}

// SeekMipLevel seek to the given subimage and mip-map level within the open.
...
func (i *ImageInput) SeekMipLevel(subimage, miplevel int, newSpec *ImageSpec) bool {
	...
	ok := C.ImageInput_seek_subimage_miplevel(i.p(), C.int(subimage), C.int(miplevel), newSpec.p())
	runtime.KeepAlive(i)
	runtime.KeepAlive(newSpec)
	return bool(ok)
}

// ReadImage reads the entire image of width * height * depth * channels into contiguous float32 pixels.
...
func (i *ImageInput) ReadImage() ([]float32, error) {
	...
	C.ImageInput_read_image_floats(i.p(), pixels_ptr)

	return pixels, i.LastError()
}

// ReadImageFormat reads the entire image of width * height * depth * channels into contiguous pixels.
// Read tiles or scanlines automatically.
//
...
func (i *ImageInput) ReadImageFormat(format TypeDesc, progress *ProgressCallback) (interface{}, error) {
	...
	C.ImageInput_read_image_format(i.p(), (C.TypeDesc)(format), ptr, cbk)

	return pixel_iface, i.LastError()
}

// ReadScanline reads the scanline that includes pixels (*,y,z), converting if necessary.
...
func (i *ImageInput) ReadScanline(y, z int) ([]float32, error) {
	...
	pixels := make([]float32, size)
	pixels_ptr := (*C.float)(unsafe.Pointer(&pixels[0]))
	C.ImageInput_read_scanline_floats(i.p(), C.int(y), C.int(z), pixels_ptr)

	return pixels, i.LastError()
}

// ReadTile reads the tile whose upper-left origin is (x,y,z),.
...
func (i *ImageInput) ReadTile(x, y, z int) ([]float32, error) {
	...
	pixels := make([]float32, size)
	pixels_ptr := (*C.float)(unsafe.Pointer(&pixels[0]))
	C.ImageInput_read_tile_floats(i.p(), C.int(x), C.int(y), C.int(z), pixels_ptr)
